  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://jatin-tagra-backend.vercel.app/api/v1/ShiftTiming/allShiftTiming`
      );
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://jatin-tagra-backend.vercel.app/api/v1/ShiftTiming/deleteShiftTiming/${id}`,
        Auth
      );
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          "https://jatin-tagra-backend.vercel.app/api/v1/ShiftTiming/AddShiftTiming",
          {
            to: fromAmount,
            from: toAmount,
            type: salaryPer,
...
export default HOC(ShiftTiming);
