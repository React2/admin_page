...
const CouponsDiscount = () => {
  ...
  const fetchCoupons = async () => {
    const url = "https://b7w3czz0m2.execute-api.ap-south-1.amazonaws.com/development/api/restaurant/getAllPlanType";
  ...
  const [Plantype, setPT] = useState("");
  console.log(Plantype);
  //const restaurantId = "63f4823419c730ffe0f71135";

  const addCoupon = async (e) => {
    e.preventDefault();
    const url = "https://b7w3czz0m2.execute-api.ap-south-1.amazonaws.com/development/api/restaurant/plantypeCreated";
    const token = localStorage.getItem("Vendortoken");
    try {
              ...
              {coupons?.map((e, i) => {
                return (
                  <tr key={i} className="tracking-wider text-gray-900 ">
                    <td className="px-4 py-3 md:text-base text-sm">{e.Plantype}</td>
                    <td
